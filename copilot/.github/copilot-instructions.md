...
begin
  ...
  try
    ...
    try
      ...
      LList := LContext.CreateEntityList<TCustomer>();
      try
...
**Identity map OFF** (`Create(AConnection, False)`) - **the caller owns** every
entity it receives:

- An entity from `Get` / `TryGet` / `CreateEntity` is yours to free, with
  `FreeEntity<T>`. The rows of `Select` / `SelectAll` go with the list from
  `CreateEntityList<T>`, and those of `RawSelect` with the list you collected
  them in - a `TTList<T>` with `FreeClone<T>` on each row when the class has a
  lazy member (rows 3 and 4 of the table below).
- A `TTLazy<T>` N:1 reference **clones the entity you assign to it**.
  ...
  - A value read through a lazy reference goes with the entity it replaces:
    after `LNext := E.Manager.Entity.Manager.Entity; E.Manager.Entity := LNext`
    `LNext` is freed with the old manager. Read `E.Manager.Entity` again.
- A `TTSession<T>` adopts the entity you pass to `Insert`, or to `Save` when it
  is not one of its clones, and a list from `CreateEntityList<T>` owns what you
  add to it: do not free those yourself, and do not read them or hand them on
  after the session or the list is gone. Hand over only an entity that is yours
  (rows 1, 5, 6 of the table below), never a row of another list. An entity
  `Insert` refused keeps its owner. In 1.0.0 the session did not free what
  `Insert` took: drop a free written for 1.0.0.
- A `TTLazyList<T>` (1:N detail) owns the child entities it loads and frees them
  with the owner.

**Identity map ON** (the default: `Create(AConnection)` or
`Create(AConnection, True)`) - **the context owns everything**. The map is a `TObjectDictionary` with `doOwnsValues`, so every
entity from `Get` / `Select` / `CreateEntity` of a type with a primary key and
no `[TJoin]` lives and dies with the context (any other type is yours as with
the map off):

- Do **not** `Free` any entity yourself - freeing the context frees them all
  (a manual `Free` is a double free). `FreeEntity<T>` is still right: it leaves
  alone what the map owns.
- Build result lists with `CreateEntityList<T>`, which does not own what the map
  holds; an owning list and the map would both free the same objects.
- `TTLazy<T>` / `TTLazyList<T>` do **not** free their entities in this mode (the
  map owns them) for a type with a primary key and no `[TJoin]`; any other type
  they free as with the map off. The same criterion holds for sessions and for
  lists from `CreateEntityList<T>`.
- A clone from `CloneEntity` or `Context.OldEntity<T>`, a `RawSelect` row kept
  in a `TTList<T>` that owns nothing, or an object you built yourself is not in
  the map, and `FreeEntity<T>` does not free it for a type with a primary key
  and no `[TJoin]`: free it with `Context.FreeClone<T>`, which frees in both
  modes. The same for such an entity passed to `TTSession<T>.Insert` or added
  to a list from `CreateEntityList<T>`: they do not free it with the map on, so
  `FreeClone<T>` it after the session or the list.
- Never `FreeClone<T>` what the map owns - an entity from `Get` / `Select` /
  `CreateEntity`, a lazy `.Entity`, a `TTLazyList` row: the map keeps the
  address, and the next read of the same key writes into freed memory.
- Assign to a `TTLazy<T>` reference only an entity the context read or created:
  with the map on it keeps the instance you pass as it is, so a clone is freed
  by nobody, and a clone the session holds in `Entities` or the `OldEntity` of
  an event leaves it on freed memory once they are destroyed.

**Who frees what, in every case** - the same table as `docs/guide/context.md`:
...
**The rule, in both modes: free every entity with `FreeEntity<T>` and build every
list of entities with `CreateEntityList<T>`.** Never a bare `LEntity.Free`, never
an owning `TTObjectList<T>.Create(True)` of your own. A rollback puts back what
Trysil wrote to the entities, so it holds them until the transaction ends: a
bare `Free` of an entity written inside that transaction makes the rollback
write into freed memory, with no exception. It does not take a transaction of
your own - an event that writes another entity runs inside the one Trysil
opened for the write that fired it. The exceptions are in the table: a clone
and an object you built yourself go through `FreeClone<T>`, and the one list
that stays yours is the result of a `RawSelect<T>`, owning for a plain DTO and
not owning for a class with a lazy member. Do not keep a pointer to a row of a
list or to a lazy `.Entity` past the next operation on its owner: read it
again. `GetOriginalEntity` answers with a row of the source list: reload that
list only after the session is freed. A list handed to `CreateSession<T>`
outlives the session: free a list of yours after the session; the list of a
lazy member is never freed by you, and the entity carrying it lives until after
the session. Reorder a list that owns its rows with
`Exchange` or `Move`, never by assigning one item over another, and before
filling a `TTList<T>` of raw select rows again `FreeClone<T>` each row.

- `Context.CreateEntityList<T>: TTList<T>` returns a list whose `OwnsObjects` is
  already set correctly for the current mode, and that tells the context about
  every entity it frees.
- `Context.FreeEntity<T>(AEntity)` forgets the entity in the context and frees
  it, unless the identity map holds entities of its type (a primary key and no
  `[TJoin]`).
- `Context.FreeClone<T>(AEntity)` does the same and frees in both modes: for
  clones, raw select rows and your own objects that no container holds.

**An entity belongs to the context that built it**: write and free it through
that context, never through another one on the same connection. Nothing checks
it, and a rollback can write into freed memory. Always build entities with
`CreateEntity<T>`.
...
nothing the context read or created needs manual freeing, and a clone goes
through `FreeClone<T>`; with it **off** you free what you created or received,
through `FreeEntity<T>`, and each lazy reference independently owns its own
clone.
...
// Read all
LList := FContext.CreateEntityList<TCustomer>();
try
  FContext.SelectAll<TCustomer>(LList);
finally
  LList.Free;
...
Clones entities on creation; `ApplyChanges` writes only what was marked with `Insert`, `Update` or `Delete` - a modified clone not passed to `Update` is not written. Full cloning is the only correct implementation in Delphi (no dynamic proxies). The session has explicit state per entity (Original/Inserted/Updated/Deleted); cloning gives isolation, not field-level diffing.
...
Not written - nothing stops `Insert`/`Update`/`Delete`/`Undelete` on a row whose class has a key and a version column, or only a key under `KeyOnly` (`Undelete` also needs `[TDeletedAt]`), so do not - and no identity map. Lazy members are mapped as on any read: a row that carries one goes in a `TTList<T>` that owns nothing and is freed with `FreeClone<T>`.
...
message of its own; with the key at zero it fails as any empty field does. The
`SELECT` is the one the lazy member would have made anyway, and it goes through
the **read** connection - so a master inserted in the same uncommitted unit of
work, on a context built with two connections, is not visible to it. A
**soft-deleted master loads and is accepted**, because a lazy member reads with
`Get<T>(ID, True)` on purpose: if your domain wants that refused, write it in a
`[TValidator]`.
...
**On a list, depth is paid per row.** At `0` a `TTLazy<T>` is written as its foreign key id **without a query**, so the client already has the identifier for free; at `1` the gate opens and every reference is resolved, which is **one query per relation per row**. A list of 50 rows with 4 relations costs 2 queries at `Create(0, False)` and 202 at `Create(1, False)`. When the list needs only the ids of the related rows, use `Create(0, False)`; when it needs their columns (the customer name next to each order), map a read-only `[TJoin]` entity and select that instead - the related columns come back in the same row from one query, with no lazy reference left to resolve.
...
- `MetadataToJSon<T>` reports each **serialized** member as `name`, `type` and, when they are not zero, `size` and `precision`; a member marked `[TJSonIgnore]` is not described at all. On a decimal column the names mislead: **`size` is the scale**, `precision` the total number of digits, so `decimal(19,4)` comes back as `"size": 4, "precision": 19`. A client validating an amount needs both.
